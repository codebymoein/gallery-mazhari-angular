...
- The WebKit checkout failure was traced to `overflow-x: clip` on the root `html/body` scroll container. After vertical scrolling, WebKit retained a valid button rectangle while hit-testing returned the root HTML element and produced an inflated document height. Using the broadly compatible `overflow-x: hidden` contract restores correct paint and hit-testing without adding a page-specific mask.
