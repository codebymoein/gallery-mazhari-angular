...
| GM-033 | EPIC-01 | Automatic browser preview for each same-repository PR before merge | L2 | P1 | IN_PROGRESS | Fixed `preview.gallerymazhari.com` slot; immutable PR browser artifacts; browser-only preview reuses reviewed V2 API and never runs unmerged backend/migrations |
